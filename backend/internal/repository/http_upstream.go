...
func (s *httpUpstreamService) DoWithTLS(req *http.Request, proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile, responseHeaderTimeout time.Duration) (*http.Response, error) {
	...
	entry, err := s.acquireClientWithTLS(proxyURL, accountID, accountConcurrency, profile, responseHeaderTimeout)
	if err != nil {
...
func (s *httpUpstreamService) acquireClientWithResponseHeaderTimeout(proxyURL string, accountID int64, accountConcurrency int, responseHeaderTimeout time.Duration) (*upstreamClientEntry, error) {
	return s.getClientEntry(proxyURL, accountID, accountConcurrency, true, true, responseHeaderTimeout)
}

// acquireClientWithTLS 获取或创建带 TLS 指纹的客户端
func (s *httpUpstreamService) acquireClientWithTLS(proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile, responseHeaderTimeout time.Duration) (*upstreamClientEntry, error) {
	return s.getClientEntryWithTLS(proxyURL, accountID, accountConcurrency, profile, true, true, responseHeaderTimeout)
}

// getClientEntryWithTLS 获取或创建带 TLS 指纹的客户端条目
// TLS 指纹客户端使用独立的缓存键，与普通客户端隔离
func (s *httpUpstreamService) getClientEntryWithTLS(proxyURL string, accountID int64, accountConcurrency int, profile *tlsfingerprint.Profile, markInFlight bool, enforceLimit bool, responseHeaderTimeout time.Duration) (*upstreamClientEntry, error) {
	...
	poolKey := s.buildPoolKey(isolation, accountConcurrency, responseHeaderTimeout) + ":tls"
	...
	settings := s.resolvePoolSettings(isolation, accountConcurrency, responseHeaderTimeout)
...
func (s *httpUpstreamService) acquireClient(proxyURL string, accountID int64, accountConcurrency int) (*upstreamClientEntry, error) {
	return s.getClientEntry(proxyURL, accountID, accountConcurrency, true, true, 0)
}
...
func (s *httpUpstreamService) getOrCreateClient(proxyURL string, accountID int64, accountConcurrency int) (*upstreamClientEntry, error) {
	return s.getClientEntry(proxyURL, accountID, accountConcurrency, false, false, 0)
}

// getClientEntry 获取或创建客户端条目
// markInFlight=true 时会标记进行中请求，用于请求路径防止被淘汰
// enforceLimit=true 时会限制客户端数量，超限且无法淘汰时返回错误
func (s *httpUpstreamService) getClientEntry(proxyURL string, accountID int64, accountConcurrency int, markInFlight bool, enforceLimit bool, responseHeaderTimeout time.Duration) (*upstreamClientEntry, error) {
	// 获取隔离模式
	isolation := s.getIsolationMode()
	// 标准化代理 URL 并解析
	proxyKey, parsedProxy, err := normalizeProxyURL(proxyURL)
	...
	// 构建连接池配置键（用于检测配置变更）
	poolKey := s.buildPoolKey(isolation, accountConcurrency, responseHeaderTimeout)
...
//   - 这确保了单账户不会占用过多连接资源
func (s *httpUpstreamService) resolvePoolSettings(isolation string, accountConcurrency int, responseHeaderTimeout time.Duration) poolSettings {
...
// 返回:
//   - string: 配置键
func (s *httpUpstreamService) buildPoolKey(isolation string, accountConcurrency int, responseHeaderTimeout time.Duration) string {
	base := "default"
	...
	if responseHeaderTimeout > 0 {
		return fmt.Sprintf("%s:rht%d", base, responseHeaderTimeout.Milliseconds())
	}
	return base
}
