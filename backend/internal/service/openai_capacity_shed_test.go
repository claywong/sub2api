...
// 用例用 400 而非 502：本 fork 已把 502 的立即摘号改为走 rateLimitService
// 的阈值计数（见 TempUnscheduleRetryableError 内注释），502 在本分支下本就不
// 写账号状态，拿它做守卫断言是空断言。400 是本分支仍然存活的即时摘号路径，
// 因此守卫组与对照组都用 400，守卫的效果才真正被测到。
func TestTempUnscheduleRetryableErrorSkipsRequestScopedTransient(t *testing.T) {
	...
	// 对照组：同样的 400 在未标记请求级瞬时故障时仍按原有语义临时摘号，
