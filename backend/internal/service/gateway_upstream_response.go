...
// isRetryLater400 判断 400 响应体是否为上游"请稍后重试"类瞬时错误。
...
func (s *GatewayService) handleStreamingResponse(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel string, mimicClaudeCode bool) (*streamingResult, error) {
	...
	for {
		...
		case ev, ok := <-events:
			...
			if ev.err != nil {
				...
				if errors.Is(ev.err, context.Canceled) || errors.Is(ev.err, context.DeadlineExceeded) {
					return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, clientDisconnect: true, capturedBody: finalizeRespCollector(respCollector)}, fmt.Errorf("stream usage incomplete: %w", ev.err)
				}
				// 客户端已通过写入失败检测到断开，上游也出错了，返回已收集的 usage
				if clientDisconnected {
					return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, clientDisconnect: true, capturedBody: finalizeRespCollector(respCollector)}, fmt.Errorf("stream usage incomplete after disconnect: %w", ev.err)
				}
				// 客户端未断开，正常的错误处理
				if errors.Is(ev.err, bufio.ErrTooLong) {
					...
					return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, capturedBody: finalizeRespCollector(respCollector)}, ev.err
				}
				...
				return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, capturedBody: finalizeRespCollector(respCollector)}, fmt.Errorf("stream read error: %w", ev.err)
			}
			line := ev.line
			trimmed := strings.TrimSpace(line)
		...
		case <-intervalCh:
			...
			if clientDisconnected {
				return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, clientDisconnect: true, capturedBody: finalizeRespCollector(respCollector)}, fmt.Errorf("stream usage incomplete after timeout")
			}
			...
			return &streamingResult{usage: usage, firstTokenMs: firstTokenMs, capturedBody: finalizeRespCollector(respCollector)}, fmt.Errorf("stream data interval timeout")
