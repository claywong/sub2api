...
func TestOpenAIGatewayService_ForwardCountTokensAsAnthropic_OAuthUnsupportedDoesNotCooldown(t *testing.T) {
	...
	require.Equal(t, 0, repo.tempUnschedCalls, "OAuth 账号缺 scope 不应触发 SetTempUnschedulable 冷却")
	require.Equal(t, 0, repo.setErrorCalls, "OAuth 账号缺 scope 不应触发 SetError 永久禁用")
}
