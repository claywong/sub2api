...
// ModelQuotaCache 受保护模型额度的 Redis 缓存接口。
type ModelQuotaCache interface {
	// GetModelQuotaUsage 读取当前窗口内的用量（窗口过期自动归零）。
	GetModelQuotaUsage(ctx context.Context, userID, groupID int64, model string) (ModelQuotaUsage, error)
	// SetModelQuotaUsage 写入初始用量（从 DB 预热时使用）。
	SetModelQuotaUsage(ctx context.Context, userID, groupID int64, model string, data ModelQuotaUsage) error
	// UpdateModelQuotaUsage 增加用量；key 不存在时返回 nil（fail-open）。
	UpdateModelQuotaUsage(ctx context.Context, userID, groupID int64, model string, cost float64) error
}
