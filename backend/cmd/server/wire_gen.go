...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	usageService := service.NewUsageService(usageLogRepository, userRepository, client, apiKeyAuthCacheInvalidator, requestLogRepository)
	...
	errorCounterCache := repository.NewErrorCounterCache(redisClient)
	openAI403CounterCache := repository.NewOpenAI403CounterCache(redisClient)
	geminiTokenCache := repository.NewGeminiTokenCache(redisClient)
	compositeTokenCacheInvalidator := service.NewCompositeTokenCacheInvalidator(geminiTokenCache)
	rateLimitService := service.ProvideRateLimitService(accountRepository, usageLogRepository, configConfig, geminiQuotaService, tempUnschedCache, timeoutCounterCache, errorCounterCache, openAI403CounterCache, settingService, compositeTokenCacheInvalidator)
	...
	openAIGatewayService := service.NewOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, requestLogRepository, serviceUserPlatformQuotaRepository)
	...
	modelQuotaCache := repository.NewModelQuotaCache(redisClient)
	subscriptionHandler := handler.NewSubscriptionHandler(subscriptionService, modelQuotaCache)
	...
	monitorGroupUsageRepository := repository.NewMonitorGroupUsageRepository(db)
	monitorGroupUsageService := service.NewMonitorGroupUsageService(monitorGroupUsageRepository, apiKeyService)
	monitorGroupUsageHandler := handler.NewMonitorGroupUsageHandler(monitorGroupUsageService)
	...
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, accountTestHealthCache, grokQuotaService)
	...
	schedulerAdminHandler := admin.NewSchedulerAdminHandler(gatewayService)
	complianceHandler := admin.NewComplianceHandler(settingService)
	auditLogRepository := repository.NewAuditLogRepository(db)
	auditLogService := service.ProvideAuditLogService(auditLogRepository, settingService)
	auditLogHandler := admin.NewAuditLogHandler(auditLogService, totpService)
	feishuOffboardHandler := admin.NewFeishuOffboardHandler()
	feishuOffboardConfigStore := service.ProvideFeishuOffboardConfigStore(settingRepository)
	feishuOffboardRepository := repository.NewFeishuOffboardRepository(db)
	feishuOffboardNotifier := service.ProvideFeishuOffboardEmailNotifier(emailService, settingRepository)
	feishuOffboardService := service.ProvideFeishuOffboardService(feishuOffboardConfigStore, feishuOffboardRepository, userRepository, opsRepository, feishuOffboardNotifier, apiKeyService, db, redisClient, configConfig)
	upstreamBillingProbeService := service.ProvideUpstreamBillingProbeService(accountRepository, accountTestService, settingService, leaderLockCache, db)
	ollamaCloudUsageService := service.ProvideOllamaCloudUsageService(accountRepository, httpUpstream, settingService, secretEncryptor, configConfig, leaderLockCache, db)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, schedulerAdminHandler, complianceHandler, auditLogHandler, feishuOffboardHandler, feishuOffboardService, upstreamBillingProbeService, ollamaCloudUsageService)
	usageRecordWorkerPool := service.NewUsageRecordWorkerPool(configConfig)
	userMsgQueueCache := repository.NewUserMsgQueueCache(redisClient)
	userMessageQueueService := service.ProvideUserMessageQueueService(userMsgQueueCache, rpmCache, configConfig)
	sessionModelLockCache := repository.NewSessionModelLockCache(redisClient)
	...
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, monitorGroupUsageHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService)
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	optionalJWTAuthMiddleware := middleware.NewOptionalJWTAuthMiddleware(authService, userService, settingService, auditLogService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(authService, userService, settingService, configConfig, auditLogService)
	...
	opsErrorWebhookDispatcher := service.ProvideOpsErrorWebhookDispatcher(configConfig, opsService)
	...
	scheduledTestRunnerService := service.ProvideScheduledTestRunnerService(scheduledTestPlanRepository, scheduledTestService, accountTestService, rateLimitService, configConfig, accountTestHealthCache)
	...
	metricCooldownRepository := repository.NewMetricCooldownRepository(db)
	metricCooldownService := service.ProvideMetricCooldownService(configConfig, metricCooldownRepository, accountRepository, rateLimitService)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsErrorWebhookDispatcher, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, cnProviderBalanceCheckService, openAICodexVersionSyncService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, channelMonitorV2Aggregator, userPlatformQuotaUsageFlusher, metricCooldownService, upstreamBillingProbeService, ollamaCloudUsageService, auditLogService, promptService)
