export default {
    settings: {
      ...
      feishuOffboard: {
        title: '飞书离职自动禁用',
        description: '每天定时查询飞书在职状态，把已离职员工的 sub2api 账号自动禁用。',
        riskNotice:
          '该功能会自动禁用用户账号。建议先开启演练模式跑几天，确认判定结果无误后再切换到真实执行。',
        loading: '加载中...',
        serviceUnavailable:
          '后端服务尚未接线（接口返回 503），当前展示的是默认值，保存会失败。请确认服务端已部署该功能。',
        enabled: '启用飞书离职同步',
        enabledHint: '关闭后定时任务不再执行，已有配置与执行历史保留。',
        schedule: 'cron 表达式',
        scheduleHint: '标准 5 位 cron 表达式，按服务器时区执行。留空则使用默认值。',
        scheduleDefaultHint: '默认每天 01:00',
        appId: '飞书 App ID',
        appIdPlaceholder: 'cli_xxxxxxxxxxxxxxxx',
        appIdHint: '飞书开放平台自建应用的 App ID，需开通通讯录读取权限。',
        appSecret: '飞书 App Secret',
        appSecretPlaceholder: '留空则不修改',
        appSecretHint: '飞书自建应用的 App Secret，保存后不再回显明文。',
        appSecretConfiguredHint: '已保存密钥。留空则保持不变，填入新值才会覆盖。',
        secretConfigured: '已配置',
        secretNotConfigured: '未配置',
        dryRun: '演练模式（dry-run）',
        dryRunHint:
          '只记录不真正禁用，用于上线前核对判定结果。开启期间定时任务照常运行并写入执行历史，但不会动任何账号。',
        circuitBreaker: '安全熔断阈值',
        circuitBreakerHint:
        ...
        runNow: '立即执行一次',
        running: '执行中...',
        runSuccess: '执行完成，已禁用 {disabled} 个账号',
        runDryRunSuccess: '演练完成，命中 {resigned} 名已离职员工（未执行禁用）',
        runFailed: '执行失败',
        saveSuccess: '配置保存成功',
        saveFailed: '配置保存失败',
        loadFailed: '加载配置失败',
        ...
        runConfirm: {
          title: '立即执行一次离职核查',
          message: '将立即查询飞书在职状态并按判定结果处理账号。请先确认执行模式。',
          dryRunLabel: '演练模式（推荐）',
          dryRunHint: '只记录判定结果，不禁用任何账号。',
          realWarning: '真实执行：命中「已离职」的账号会被立即禁用，请确认判定规则已核对无误。',
          confirm: '开始执行',
        },
        history: {
          title: '执行历史',
          description: '最近的执行记录。点击某行查看该次的每人判定明细。',
          refresh: '刷新',
          empty: '暂无执行记录',
          loadFailed: '加载执行历史失败',
          viewDetail: '查看明细',
          ...
          modeReal: '真实执行',
          circuitBroken: '安全熔断',
          circuitBrokenHint: '命中过多，已阻止执行。请检查飞书数据是否异常后再手动处理。',
          columns: {
            runAt: '执行时间',
            trigger: '触发方式',
            mode: '模式',
        ...
        flags: {
          ...
          no: '否',
          is_resigned: '已离职',
          is_resignedHint: '判定依据：为「是」时视为已离职。',
          is_activated: '已激活',
          is_activatedHint: '仅供参考，不作为判定依据。离职后飞书账号仍可能保持激活。',
          is_exited: '已退出租户',
          is_exitedHint: '判定依据：为「是」时视为已离开企业。',
          is_frozen: '已冻结',
          is_frozenHint: '冻结/暂停状态，归入「需人工判断」，不会自动禁用。',
          is_unjoin: '未加入',
          is_unjoinHint: '待确认入职，不会自动禁用。',
        },
        detail: {
          title: '判定明细',
          empty: '该次执行没有判定明细',
