...
export default {
  dlp: {
    ...
    disposition: '命中处置',
    recordRegexHits: '把命中但放行的也记为事件',
    recordRegexHitsHint:
      '默认只有确认为敏感的命中才写事件。打开后，模型判为误报、以及确认服务不可用导致放行的命中也会记录：前者用于统计哪条规则误报多，后者是漏放风险的线索。列表里都显示为「通过」，误报是「低」、降级放行是「中」，筛后者即可复查。',
    recordRegexHitsWarning:
      '误报通常远多于真命中，而安全事件表没有自动清理，长期打开会让它持续增长。建议调规则时临时打开，调完关掉；清理用事件列表的「按条件删除」。',
