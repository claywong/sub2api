export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      metricCooldown: {
        title: '指标冷却（覆盖全局默认）',
        hint: '后端会按全局 cron 周期扫描该账号窗口内的 4 个指标，任一命中阈值即标记 manual-cooldown，到期后调度器自动放出，定时测试 auto-recover 不会清除。留空字段继承全局默认。',
        accountDisabled: '为此账号关闭指标冷却',
        accountDisabledHint: '关闭后此账号不会被扫描评估，其他 override 字段将被忽略。',
        windowMinutes: '评估窗口（分钟）',
        minSampleCount: '样本数下限',
        cooldownHours: '冷却时长（小时）',
