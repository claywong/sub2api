<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select v-if="modelNames && modelNames.length > 0" :model-value="filters.model_name || ''" class="w-48" :options="mOpts" @update:model-value="updateModelName" @change="$emit('change')" />
  </div>
</template>
...
const props = defineProps<{ searchQuery: string; filters: Record<string, any>; groups?: AdminGroup[]; modelNames?: string[] }>()
...
const updateModelName = (value: string | number | boolean | null) => { emit('update:filters', { ...props.filters, model_name: value }) }
...
const mOpts = computed(() => [
  ...
  ...(props.modelNames || []).map(m => ({ value: m, label: m }))
])
</script>
