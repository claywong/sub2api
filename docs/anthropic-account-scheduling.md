...
Anthropic/Claude 的账号调度与 OpenAI 调度器**完全独立**，没有 OpenAI 那套 EWMA 加权打分机制，是一套基于**多层过滤 + 质量分桶 + 负载/LRU**的确定性漏斗调度体系。整个调度入口为 `SelectAccountWithLoadAwareness`（负载感知路径，主路径）和 `SelectAccountForModel`（简单路径，无并发服务时使用）。

质量分桶（TTFT / OTPS / CacheHit）仅在 `gateway.scheduling.account_health.enabled=true` 时生效；关闭时分桶缓存为 `nil`，过滤函数透传，Layer 2 退化为 `Priority → LoadRate → LRU`。
...
路由候选全不可用 → Fall through 到 Layer 1.5/Layer 2
```
...
分层过滤选择（六级漏斗）：
  ...
  2. filterByMinTTFTBucket     → 只保留 TTFT 分桶最优的账号（account+model 维度）
  3. filterByMinOTPSBucket     → 只保留 OTPS 分桶最优的账号（account+model 维度）
  4. filterByMinCacheHitBucket → 只保留缓存命中率分桶最优的账号（account+model 维度）
  5. filterByMinLoadRate       → 只保留负载率最低的账号
  6. selectByLRU               → 选择 LastUsedAt 最久的账号
        │
        ▼
tryAcquireAccountSlot
  ├── 成功 → checkAndRegisterSession
...
当 `account_health.enabled=false` 时，`qualityBucketCache()` 返回 `nil`，TTFT / OTPS / CacheHit 三个分桶过滤函数直接返回原候选集，不改变选择结果。
...
| `isAccountSchedulableForHealth()` | Anthropic | 健康感知开启时，`HealthStickyOnly` 只允许 sticky，`HealthExcluded` 全拒绝 |
...
在通过所有过滤条件的账号中，Layer 2 选择规则为：

```
1. Priority 数值越小 → 优先级越高（必选）
2. TTFT 分桶越小 → 越优先（仅 account_health.enabled=true）
3. OTPS 分桶越小 → 越优先（仅 account_health.enabled=true）
4. CacheHit 分桶越小 → 越优先（仅 account_health.enabled=true）
5. LoadRate 越低 → 越优先（负载感知路径）
6. LastUsedAt:
   ├── nil（从未使用）> 有使用记录（LRU）
   └── 有记录时：越早使用的越优先
```
...
质量分桶由 `AccountModelQualityCache` 维护，key 为 `(account_id, requested_model)`，只保存在进程内，重启后清空。无样本时分桶视为最优，避免冷启动惩罚。

| 分桶 | 规则 | 说明 |
|------|------|------|
| TTFT | `<4s → 0`，`4s` 起每 `3s` 一档 | 数字越小越优先 |
| OTPS | `>=60 → 0`，`40-60 → 1`，`20-40 → 2`，`<20 → 3` | tokens/s 越高越优先 |
| CacheHit | `>=80% → 0`，`60-80% → 1`，`40-60% → 2`，`20-40% → 3`，`<20% → 4` | 缓存命中率越高越优先 |
...
| 选择算法 | 多层过滤 + 质量分桶 + 负载/LRU 确定性漏斗 | 5 维加权打分 + top-K 加权随机 |
| TTFT | 健康感知开启时进入 account+model 分桶与 HealthVerdict | EWMA 实时统计，参与打分 |
| ErrorRate | 健康感知开启时进入 HealthVerdict 三态，不参与加权打分 | EWMA 实时统计，参与打分 |
| 负载感知 | LoadRate 位于质量分桶之后 | Load + Queue 共两个打分维度 |
| 模型路由 | 支持（仅 Anthropic 平台） | 不支持 |
| 会话限制 | OAuth/SetupToken 支持 max_sessions | 不涉及 |
| 窗口费用限制 | OAuth/SetupToken 支持 | 不涉及 |
| 冷启动处理 | 无质量样本视为最优桶；从未用账号 LRU 最优先 | TTFT=NaN → 退化为 0.5 中性值 |

---

## 健康感知调度（可选）

启用 `gateway.scheduling.account_health.enabled=true` 后，Layer 2 会新增健康过滤和质量分桶：

| 检查 | 语义 |
|------|------|
| `isAccountSchedulableForHealth(acc, false)` | HealthStickyOnly / Excluded 的账号不进入新会话候选集 |
| `filterByMinTTFTBucket` / `filterByMinOTPSBucket` / `filterByMinCacheHitBucket` | 在同优先级账号中优先选择当前模型质量分桶更好的账号 |

Layer 1.5（粘性会话）同样增加了 `healthOK` 检查（`isSticky=true`），HealthStickyOnly 状态下粘性会话仍可继续服务，详见 `anthropic-scheduling-health-aware.md`。

注意：模型路由 Layer 1 当前按路由集合过滤和排序，不走质量分桶，也不调用 `isAccountSchedulableForHealth`；路由集合不可用时才 fall through 到 Layer 2。

---

## 已知缺陷与改进空间

1. **健康感知关闭时无质量排序**：调度器不感知账号的响应延时（TTFT）、输出速度（OTPS）和缓存命中率，所有通过过滤的账号按 `Priority → LoadRate → LRU` 选择。启用健康感知后，同优先级账号会先按 TTFT/OTPS/CacheHit 分桶过滤，且 TTFT/OTPS 过差会影响 HealthVerdict。

2. **ErrorRate 软降权仅在健康感知开启时生效**：关闭时错误只有两种结果——账号被标记为不可调度（过滤掉）或正常可用，没有中间态。

3. **冷启动无先验数据**：进程重启后健康缓存和模型质量缓存为空，所有账号视为 HealthOK，质量分桶视为最优，不惩罚新账号；LRU 对冷账号天然友好。
