...
| 状态 | 新会话（Layer 2） | 粘性会话（Layer 1.5） |
|------|-----------------|---------------------|
| `HealthOK` | 放行 | 放行 |
| `HealthStickyOnly` | 拒绝 | 放行 |
| `HealthExcluded` | 拒绝 | 拒绝 |

需要 `gateway.scheduling.account_health.enabled=true` 才生效（默认 false）。

### 状态变化日志

`HealthVerdictWithChange` 在状态切换时打一次日志（去抖），避免重复刷屏：

```
[HealthVerdictChange] account_id=123 -> StickyOnly (sticky_path=false)
```
...
### 退避状态机（`AccountHealthConfig`）

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `HardFilterThreshold` | 2 | ConsecFails 达到此值，新会话硬过滤（`PassesHardFilter=false`） |
| `TempUnschedThreshold` | 4 | ConsecFails 达到此值，触发 TempUnschedulable |
| `TempUnschedInitDuration` | 30min | TempUnschedulable 初始时长 |
| `TempUnschedMaxDuration` | 60min | TempUnschedulable 最大时长 |
| `RetryIntervalStep` | 15s | 退火补测间隔步长（每次失败翻倍） |
| `RetryIntervalMax` | 5min | 退火补测间隔上限 |
| `SlowThresholdMs` | 20000ms | 慢请求判定阈值（DurationMs ≥ 此值计入 slowCount） |

### HealthVerdict 三态阈值（`SchedulingHealthConfig`）

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `WindowMinutes` | 10 | 滑动窗口长度（分钟） |
| `MinSamples` | 5 | 触发判定的最小样本数 |
| `ErrCountSoft` | 5 | 错误数 ≥ 此值 → StickyOnly |
| `ErrCountHard` | 10 | 错误数 ≥ 此值 → Excluded |
| `ErrRateSoft` | 0.3 | 错误率 ≥ 此值 → StickyOnly |
| `ErrRateHard` | 0.5 | 错误率 ≥ 此值 → Excluded |
| `TTFTStickyOnlyMs` | 10000ms | TTFTAvg ≥ 此值 → StickyOnly |
| `OTPSStickyOnlyMin` | 10 tokens/s | OTPSAvg < 此值（且有样本）→ StickyOnly |
